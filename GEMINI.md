<contextstream>
<!-- contextstream-rules-hash: 15504e53a68292a2 -->
# Workspace: StreamBox
# Project: lucky5-v8
...
- "create capsule" / "context capsule" / "capsule link" / "share this context with another agent" → `capsule(action="create", scope="session", session_id="<current session id>", purpose="handoff")`. Session creates automatically mint a safe external-agent share and return the capsule id plus Agent URL and Dashboard URL; use `audience="self"` only when the user explicitly asks for no share link. **NEVER substitute a handoff entity, design spec, document, or prose summary for a requested capsule.**
...
- "create handoff" / "package context for handoff" → `entity(kind="handoff", action="create", body={"title": "...", "summary": "...", "scope": "...", "to_user_id": "...", "next_steps": [...]})` for the workflow shell; if the user requests a portable bundle, capsule, or share link, use `capsule(...)` for the artifact (and pair it with the handoff only when both are requested)
...
**IMPORTANT: Indexing works in every transport, but the mechanics differ. A local MCP process ingests folders directly; a hosted/remote gateway reports committed server-side index state (editor hooks / ContextStream Desktop keep it fresh). Treat a `requires_local_ingest` response as instructions to follow — never retry-loop the index call.**
...
**IMPORTANT: Indexing and ingest are available in every configuration, but the mechanics depend on where the MCP process runs. A local (stdio) process ingests folders directly. A hosted/remote gateway cannot read this machine's disk: `project(action="index")` then reports committed server-side index state, and local file changes are ingested by editor hooks or ContextStream Desktop. A `requires_local_ingest` response is guidance to run a local ingest path — not an error to retry.**
