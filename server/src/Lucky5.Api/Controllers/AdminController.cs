    ...
    ICabinetDeviceAuthService cabinetDeviceAuthService) : ControllerBase
{
    ...
    [HttpPost("users/create")]
        ...
        public async Task<ActionResult<ApiResponse<AdminUserDto>>> BulkAssignAgent([FromBody] Lucky5.Application.Requests.BulkAssignAgentRequest request, CancellationToken cancellationToken)
        {
            var adminId = HttpContext.RequireAdminRole();
            if (request.UserIds == null || request.UserIds.Count == 0)
                return BadRequest(ApiResponse<AdminUserDto>.Fail("UserIds array cannot be empty", traceId: HttpContext.TraceIdentifier));

            var user = await adminService.BulkAssignAgentAsync(adminId, request, cancellationToken);
            await auditService.AppendAsync(new AdminAuditWriteDto(
                adminId,
                "admin",
                "user.assign_agent",
                ...
                Outcome: "succeeded",
                Reason: $"Assigned agent {request.AgentId} to {request.UserIds.Count} users",
                Metadata: new Dictionary<string, string> { { "count", request.UserIds.Count.ToString() } }),
                cancellationToken);
