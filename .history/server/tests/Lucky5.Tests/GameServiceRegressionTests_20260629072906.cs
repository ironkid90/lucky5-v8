...
public static class GameServiceRegressionTests
{
	...
	private static async Task JackpotSnapshotsExposeAuthoritativeMachineIdentityAsync(List<string> failures)
	{
		...
		Assert(
			failures,
			"In-memory machine seed data should define authoritative serial, serie, and variant state values.",
			!string.IsNullOrWhiteSpace(machine.MachineSerial)
			&& !string.IsNullOrWhiteSpace(machine.MachineSerie)
			&& !string.IsNullOrWhiteSpace(machine.VariantState));
		...
		Assert(
			...
			&& string.Equals(jackpots.MachineKent, machine.VariantState.Contains("MachineKent") ? System.Text.Json.JsonDocument.Parse(machine.VariantState).RootElement.GetProperty("MachineKent").GetString() : string.Empty, StringComparison.Ordinal));
		...
		Assert(
			...
			&& string.Equals(ledger.MachineKent, machine.VariantState.Contains("MachineKent") ? System.Text.Json.JsonDocument.Parse(machine.VariantState).RootElement.GetProperty("MachineKent").GetString() : string.Empty, StringComparison.Ordinal));
	}
	...
	private static async Task GetActiveRoundRestoresActiveDoubleUpPhaseAsync(List<string> failures)
	{
		...
		duSession = duSession with
		{
			...
			BoardHandRank = HandCategory.TwoPair,
