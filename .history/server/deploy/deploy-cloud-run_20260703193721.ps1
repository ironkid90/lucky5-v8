...
if ($MakePublic) {
    ...
    gcloud run services add-iam-policy-binding $Service --project $ProjectId --region $Region `
        --member allUsers --role roles/run.invoker 2>$null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "    Initial attempt failed - this project likely enforces the domain-restricted-sharing" -ForegroundColor Yellow
        Write-Host "    org policy (iam.allowedPolicyMemberDomains). Attempting a project-level override..." -ForegroundColor Yellow
...
"@ | Set-Content -Path $policyFile -Encoding utf8
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "Could not grant public access even after the org policy override attempt. You (or an org admin) may need to loosen constraints/iam.allowedPolicyMemberDomains manually, or accept IAM-restricted access."
        }
    }
}
